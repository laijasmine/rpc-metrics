...
```{r}
sum(volunteer_sheet_clean$Hours, na.rm = TRUE)

length(unique(volunteer_sheet_clean$`Column 2`))
```
...
volunteer_branch_plot <- volunteer_total_hours |>
  left_join(membership, by = join_by(`Column 2` == `Full Name`)) |>
  filter(
    !(`Assigned Branch` %in%
      c("Exempt", "Operations - Treasurer", "Operations - Tres assistant"))
  ) |>
  ggplot(aes(x = member_factor, y = `Total hours`)) +
  geom_col(aes(fill = `Assigned Branch`)) +
...
ggplotly(volunteer_branch_plot)
```
